...
public class ServiceReservationColis implements IReservation<Reservation_Colis> {
  ...
  @Override
    public List<Reservation_Colis> afficherReservationColis(int id) {
        ...
        try {
          //  SELECT * FROM offre_colis OC INNER JOIN fos_user fos on OC.idU=fos.id 
            String requete = "SELECT * FROM reservation_colis RC INNER JOIN offre_colis OC on RC.idOffre=OC.id_Offre_Col INNER JOIN fos_user fos on OC.idU=fos.id where idUR ="+id+"";
            PreparedStatement pst = cnx.prepareStatement(requete);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {

               list.add(new Reservation_Colis(rs.getInt(1),rs.getDouble(2),rs.getDouble(3),rs.getDouble(4),rs.getInt(5),rs.getInt(6),rs.getDouble(7),rs.getString("Lieu_Arrive"),rs.getString("Lieu_Depart"),rs.getString("username")));
            }
    ...
    public List<Reservation_Colis> afficherReservationColisAll() {
        ...
        try {
            String requete = "SELECT * FROM reservation_colis RC INNER JOIN offre_colis OC on RC.idOffre=OC.id_Offre_Col INNER JOIN fos_user fos on OC.idU=fos.id ";
            PreparedStatement pst = cnx.prepareStatement(requete);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {

                    //public Reservation_Colis( int id, int id_Offre, double HauteurResv, double LargeurResv, double LongueurResv, int PrixResv) {
               list.add(new Reservation_Colis(rs.getInt(1),rs.getDouble(2),rs.getDouble(3),rs.getDouble(4),rs.getInt(5),rs.getInt(6),rs.getDouble(7),rs.getString("Lieu_Arrive"),rs.getString("Lieu_Depart"),rs.getString("username"),rs.getInt("tel")));
            }
    ...
    public void ajouterOffreReservation(Reservation_Colis O) {
       ...
       //  String requete1 = "UPDATE  offre_colis SET Hauteur="+(GetHauteur(id_Offre)-GetHauteurResv(id_Offre))+" WHERE id_Offre_Col="+id_Offre+"";
           String requete = "INSERT INTO Reservation_Colis (hauteur_resv,largeur_resv,prix,idUR,idOffre,longueur_resv) VALUES (?,?,Round('"+O.getLargeurResv()*O.getHauteurResv()*O.getLongueurResv()*GetPrix(O.getId_Offre())+"',2),?,?,?) ";
           PreparedStatement pst = cnx.prepareStatement(requete);
		...
		File qrFile = new File(filePath);
            ...
            {
                System.out.println("you can't Reserve this offre");
            }
//////////////
